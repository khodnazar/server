...
module.exports = {
  ...
  methods: {
    ...
    fetchAliasesForService(service) {
      return this.broker.call(`v1.api.listAliases`);
    },
    ...
    async collectRoutes(nodes) {
      ...
      const myNodeName = process.env.MOLECULER_NODEID;

      for (const node of nodes) {
        if (node.nodes.includes(myNodeName) == false) {
          continue;
        }
        ...
        if (node.settings && node.settings.routes) {
          ...
          const hasAutoAliases = node.settings.routes.some((route) =>
            route ? route.autoAliases : false
          );
          if (hasAutoAliases) {
      ...
      // remove /api/openapi from routes if exists
      if (routes["openapi.generateDocs"]) {
        delete routes["openapi.generateDocs"];
      }

      if (routes["openapi.assets"]) {
        delete routes["openapi.assets"];
      }

      if (routes["openapi.ui"]) {
        delete routes["openapi.ui"];
      }
    ...
    attachRoutesToDoc(routes, doc) {
      ...
      for (const action in routes) {
        ...
        for (const path of paths) {
          ...
          // add x-creator in header as string input
          doc.paths[openapiPath][method].parameters.push({
            in: "header",
            name: "x-creator",
            schema: {
          ...
          // add x-i18n in header and user can just pick fa or en
          doc.paths[openapiPath][method].parameters.push({
            in: "header",
            name: "x-i18n",
            schema: {
    ...
    getTypeAndExample(node) {
      ...
      if (
        node.pattern &&
        (node.pattern.length > 0 || node.pattern.source && node.pattern.source.length > 0)
      ) {
    ...
    expandShortDefinition(shortDefinition) {
      ...
      if(typeof shortDefinition != "string") return node;
