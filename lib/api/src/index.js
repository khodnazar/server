	...
	methods: {
		...
		routeHandler(ctx, route, req, res, foundAlias) {
			...
			return new this.Promise(async (resolve, reject) => {
				...
				try {
					...
					// @MINE
					const reqPath = req.parsedUrl.split("?")[0];

					// check if in this.$routes[reqPath] and actions exists or not
					if (this.$routes[reqPath]) {
						action = this.$routes[reqPath];
					} else {
						// all actions
						const actions = this.broker.registry.getActionList({});

						for (let item of actions) {
							if (item.action && item.action.rest) {
								const method = item.action.rest.split(" ")[0];

								if (method == req.method) {
									let path = item.action.rest.split(" ")[1];
									// name without last . after
									...
									const _paths = path
										.split("/")
										.filter((item) => item != "");
									const _reqPaths = reqPath
										.split("/")
										.filter((item) => item != "");

									if (_paths.length == _reqPaths.length) {
										let valid = true;
										for (
											let i = 0;
											i < _paths.length;
		...
		regenerateAutoAliases(route) {
			...
			services.forEach((service) => {
				if (service.nodes.includes(myNodeName) == false) {
					return;
				}
